...
// 标记文件兼作构建信息：上面那段清空前的安全检查靠它认门，
// test/check-prerender.mjs 也从这里读 base / siteUrl，
// 这样根路径构建和子路径构建（fork 的项目站）用的是同一套断言。
fs.writeFileSync(
  path.join(OUT, MARKER),
  JSON.stringify({ by: "site/scripts/build-static.mjs", base: BASE, siteUrl: SITE_URL }, null, 2) + "\n"
);
